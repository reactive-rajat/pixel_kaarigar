 ...
 */
export const TextCardGrid = ({ id, items = [], columns = 2, className = "", cardClassName = "" }) => {
  ...
  return (
    <div id={id} className={`grid ${getGridColsClass(columns)} gap-6 ${className}`}>
      {items.map((item, index) => (
        <div key={index} className={`card-1 card-sm border ${cardClassName}`}>
